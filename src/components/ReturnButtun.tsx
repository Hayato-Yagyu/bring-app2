...
const ReturnButton: React.FC<Props> = ({ rowId, rowData }) => {
  const { user } = useUser();
  ...
  // ダイアログを開いたタイミングで approvers をロード（毎回最新を取りたいなら open を依存に）
  useEffect(() => {
    if (!open) return;
    const load = async () => {
    ...
    <div>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="xs">
        ...
        <DialogActions>
          ...
          <Button onClick={handleApproval} variant="outlined" color="error" sx={{ width: 150 }} disabled={sending || !selectedEmail}>
            {sending ? "送信中..." : "申請"}
          </Button>
        </DialogActions>
      </Dialog>
