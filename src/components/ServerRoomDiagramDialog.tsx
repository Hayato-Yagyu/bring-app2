...
export const ServerRoomDiagramDialog: React.FC<Props> = ({ open, onClose, equipments, currentUid }) => {
  ...
  // クリック時：スロットの種別に応じて候補（サーバー or ルーター）を決定
  const handleSlotClick = (slotId: string) => {
    setActiveSlotId(slotId);
    const eqId = layout.slots[slotId]?.equipmentId || null;
    const pool = isRouterSlot(slotId) ? routerOptions : serverOptions;
  ...
  // ========= 共通：矩形描画 =========
  const Rect: React.FC<{ id: string; x: number; y: number; w: number; h: number; labelOverride?: string }> = ({ id, x, y, w, h, labelOverride }) => {
  ...
  const SvgBoth: React.FC = () => (
    <svg width="100%" viewBox={`0 0 ${totalW} ${totalH}`} style={{ border: "1px solid #ddd", borderRadius: 6 }} preserveAspectRatio="xMidYMid meet">
      ...
      <g transform={`translate(${leftOriginX},0)`}>
        ...
        {/* ★ 追加：左面1段目（ネットワーク棚）の割り当てリストを右横に表示 */}
  ...
  const optionPool = isRouterSlot(activeSlotId) ? routerOptions : serverOptions;
  const acLabel = isRouterSlot(activeSlotId) ? "割り当てる機器（assetNo／ルーターのみ）" : "割り当てる機器（assetNo／サーバーのみ）";
  const noOptionsText = isRouterSlot(activeSlotId) ? "ルーターが見つかりません" : "サーバーが見つかりません";
  ...
  return (
    <>
      ...
      {/* 編集用ダイアログ（サーバー or ルーターに自動切替し、assetNo表示） */}
      <Dialog
        open={!!activeSlotId}
        onClose={() => {
          setActiveSlotId(null);
