...
const TEMPLATE_APPROVED_ID = "template_svuz27k"; // ← 承認通知テンプレIDに変更
const TEMPLATE_REJECTED_ID = "template_rejected_notify"; // ← 却下通知テンプレIDに変更
...
const ApprovalsDialog: React.FC<Props> = ({ open, onClose }) => {
  ...
  // 変更承認 → posts を snapshot で更新し、登録承認情報を付与
  ...
  // 返却承認 → posts の返却確認情報を更新
  const approveReturn = async (r: ApprovalRow) => {
    const p = {
  ...
  const handleApprove = async (r: ApprovalRow) => {
    ...
    setBusyId(r.id);
    try {
      if (r.type === "change") await approveChange(r);
      if (r.type === "return") await approveReturn(r);
